...
*Diese Baseline wird durch die [MDM-Sicherheitsbaseline für Mai 2019 ersetzt](security-baseline-settings-mdm-all.md?pivots=mdm-may-2019)* .
...
- **Authentifizierungsebene**  
  ...
  - *Send LM and NTLM responses* (LM- und NTLM-Antworten senden): Clients verwenden die NTLM-Authentifizierung und nie die NTLMv2-Sitzungssicherheit. Domänencontroller akzeptieren die LM-, NTLM- und NTLMv2-Authentifizierungen. 
  - *Send LM and NTLM - NTLMv2 if negotiated* (LM und NTLM senden – NTLMv2 bei Aushandlung): Clients verwenden die LM- und NTLM-Authentifizierung sowie die NTLMv2-Sitzungssicherheit, wenn der Server diese unterstützt. Domänencontroller akzeptieren die LM-, NTLM- und NTLMv2-Authentifizierungen. 
...
- **Administrator elevation prompt behavior** (Verhalten für Administratoren bei der Eingabeaufforderung für erhöhte Rechte)  
  ...
  - *Aufforderung zur Eingabe von Anmeldeinformationen auf sicherem Desktop*: Wenn für einen Vorgang erhöhte Rechte erforderlich sind, wird der Benutzer auf dem sicheren Desktop zur Eingabe eines Benutzernamens und Kennworts eines Benutzers mit erhöhten Rechten aufgefordert. Wenn der Benutzer gültige Anmeldeinformationen eingibt, wird der Vorgang mit dem höchsten verfügbaren Recht des Benutzers fortgesetzt. 
  - *Zustimmungsaufforderung für sicheren Desktop*: Wenn für einen Vorgang erhöhte Rechte erforderlich sind, wird der Benutzer auf dem sicheren Desktop zur Auswahl von „Zulassen“ oder „Verweigern“ aufgefordert. Wenn der Benutzer „Zulassen“ auswählt, wird der Vorgang mit dem höchsten verfügbaren Recht des Benutzers fortgesetzt. 
  ...
  - *Zustimmungsaufforderung*: Wenn für einen Vorgang erhöhte Rechte erforderlich sind, wird der Benutzer zur Auswahl von „Zulassen“ oder „Verweigern“ aufgefordert. Wenn der Benutzer „Zulassen“ auswählt, wird der Vorgang mit dem höchsten verfügbaren Recht des Benutzers fortgesetzt.  
  - *Zustimmungsaufforderung für Nicht-Windows-Binärdateien*: (Standardeinstellung) Wenn erhöhte Rechte für einen Vorgang für eine nicht von Microsoft stammende Anwendung erforderlich sind, wird der Benutzer auf dem sicheren Desktop aufgefordert, entweder „Zulassen“ oder „Verweigern“ auszuwählen. Wenn der Benutzer „Zulassen“ auswählt, wird der Vorgang mit dem höchsten verfügbaren Recht des Benutzers fortgesetzt.   
...
- **Minimum session security for NTLM SSP based clients** (Minimale Sitzungssicherheit für NTLM-basierte SSP-Clients)  
  ...
  - *128-Bit-Verschlüsselung erfordern*: Die Verbindung schlägt fehl, wenn die keine starke Verschlüsselung (128-Bit) ausgehandelt wird.
...
- **Prevent anonymous enumeration of SAM accounts** (Anonyme Enumerationen von SAM-Konten verhindern)  
  ...
  - *Ja*: Die Enumeration von SAM-Konten wird nicht zugelassen. Diese Option ersetzt die Option „Everyone with Authenticated Users“ (Jeder mit authentifizierten Benutzern) in den Sicherheitsberechtigungen für Ressourcen.
...
- **Starttreiberinitialisierung für Systemstart**  
  ...
  - *Food* (Gut): Der Treiber wurde signiert und nicht manipuliert.  
  ...
  - *Bad, but required for boot* (Schlecht, aber für Start erforderlich): Der Treiber wurde als Schadsoftware identifiziert, doch der Computer kann nicht erfolgreich gestartet werden, ohne diesen Treiber zu laden. 
...
[Ändern der Baselineversion für ein Profil](security-baselines.md#change-the-baseline-version-for-a-profile)
